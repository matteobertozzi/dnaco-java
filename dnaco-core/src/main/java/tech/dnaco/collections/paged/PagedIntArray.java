...
import tech.dnaco.collections.ArrayUtil.IntArrayConsumer;
import tech.dnaco.util.BitUtil;

public class PagedIntArray {
  ...
  public PagedIntArray() {
    this(DEFAULT_PAGE_SIZE);
  }

  public PagedIntArray(final int pageSize) {
    this.pageSize = BitUtil.nextPow2(pageSize);

    this.pageItems = 0;
    this.pageCount = 1;
    this.lastPage = new int[this.pageSize];
    this.pages = null;
  }
  ...
  public void forEach(final int off, int len, final IntArrayConsumer consumer) {
    if (len == 0) return;
  ...
  public void add(final int[] buf, int off, int len) {
    while (len > 0) {
  ...
  private void rollPage() {
    if (pages == null) {
      pages = new int[DEFAULT_PAGES_GROWTH][];
      pages[0] = lastPage;
