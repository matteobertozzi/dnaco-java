...
public class IntHashSet {
  private static final int SPECIAL_VALUE = Integer.MIN_VALUE;
  ...
  public IntHashSet(final int initialSize) {
    table = new int[BitUtil.nextPow2(initialSize)];
    Arrays.fill(table, SPECIAL_VALUE);
    containsSpecialValue = false;
    count = 0;
  }

  public static IntHashSet fromArray(final int[] values) {
    final IntHashSet hashSet = new IntHashSet(Math.round(values.length * 1.5f));
  ...
  private static boolean add(final int[] table, final int value) {
    final int mask = table.length - 1;
    int index = Hashing.hash(value, mask);
