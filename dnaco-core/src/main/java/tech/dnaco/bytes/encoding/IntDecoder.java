...
public abstract class IntDecoder {
  ...
  // ================================================================================
  public long readUnsignedVarLong(final byte[] buf, final int off) {
    long value = 0;
    int shift = 0;
    long b;
  ...
  public int readUnsignedVarInt(final byte[] buf, final int off) {
    return (int) readUnsignedVarLong(buf, off);
  }

  public long readUnsignedVarLong(final InputStream stream) throws IOException {
    long value = 0;
    int shift = 0;
    long b;
  ...
  public int readUnsignedVarInt(final InputStream stream) throws IOException {
  ...
  private static final class BigEndian extends IntDecoder {
    ...
    public long readFixed(final byte[] buf, int off, final int bytesWidth) {
      ...
      switch (bytesWidth) {
        ...
        case 1: result += (((long)buf[off++] & 0xff) <<  0);
      }
      return result;
    }
  ...
  private static final class LittleEndian extends IntDecoder {
    ...
    public long readFixed(final InputStream stream, final int bytesWidth) throws IOException {
      long result = 0;
      for (int i = 0; i < bytesWidth; ++i) {
        result += (((long)(stream.read() & 0xff)) << (i << 3));
      }
      return result;
    }
  }
