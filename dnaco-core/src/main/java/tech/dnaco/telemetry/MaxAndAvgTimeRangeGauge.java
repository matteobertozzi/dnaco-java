...
public class MaxAndAvgTimeRangeGauge implements TelemetryCollector {
  ...
  public static void main(final String[] args) {
    final MaxAndAvgTimeRangeGauge trc = new MaxAndAvgTimeRangeGauge(10, 1, TimeUnit.SECONDS);
    final long now = trc.lastInterval;
    System.out.println(new Date(trc.lastInterval));
    System.out.println(Arrays.toString(trc.ringAvg));
    System.out.println(Arrays.toString(trc.ringMax));
    for (int i = 0; i < 10; ++i) {
      trc.set(now + (1000 * i), 1 + i);
      trc.set(now + (1000 * i), (1 + i) * 2);
    }
    System.out.println(new Date(trc.lastInterval));
    System.out.println(Arrays.toString(trc.ringAvg));
    System.out.println(Arrays.toString(trc.ringMax));
