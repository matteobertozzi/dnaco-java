...
public final class Statistics {
  ...
  public static float percentile(final float p, final long[] bounds, final long[] events, final long maxValue, final long nevents) {
    ...
    for (int i = 0; (i < bounds.length) && (maxValue > bounds[i]); ++i) {
      ...
      if (xsum >= threshold) {
        ...
        float pos = 0;
        final float right_left_diff = xsum - left_xsum;
  ...
  //  Others...
  // ================================================================================
  public static long sum(final long... data) {
    return sum(data, 0, data.length);
  }

  public static long sum(final long[] data, final int offset, final int length) {
    long xsum = 0;
  ...
  public static float rateSec(final long[] counters, final int interval, final TimeUnit intervalUnit) {
    return sum(counters, counters.length - interval, counters.length) / (interval * intervalUnit.toSeconds(1));
  }

  public static float rateMin(final long[] counters, final int interval, final TimeUnit intervalUnit) {
    return sum(counters, counters.length - interval, counters.length) / (interval * intervalUnit.toMinutes(1));
  }
}
